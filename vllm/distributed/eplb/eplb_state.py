...
from .eplb_thread.eplb_thread import EPLBThread
...
class EplbState:
        ...
        if self._async_processor and self._async_processor.has_pending_task:
            ...
            try:
                if self._async_processor.step():
                    logger.error("EPLBState step2")
                    # Process results
                    self._process_async_result()
                    logger.error("EPLBState step3")
    ...
    def _process_async_result(self):
        logger.error("_process_async_result step0")
        ...
        logger.error("_process_async_result step1")
        ...
        logger.error("_process_async_result step2")
        ...
        logger.error("_process_async_result step3")
        logger.error(str(new_physical_to_logical_map))
        logger.error(str(new_logical_to_physical_map))
        logger.error(str(new_logical_replica_count))
        ...
        logger.error("_process_async_result step4")
        if not is_profile:
